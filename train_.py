...
def evaluate(model, dataloader, eval_dset, gamma_r, use_feat_loss, output, ckpt, compute_acc = False):
    ...
    if valid:
        ...
        for step, batch in enumerate(dataloader):
            ...
            #input from dataset
            input, GT = batch
            # question_raw, question_id, _, _, _ = input
            ...
            save_visual = []
            for i in range(bs):
                json_string += '{"question_id":'+str(int(question_id[i]))+',"answer":"'+label2ans[prediction[i]]+'"}, '
            #     save_visual.append([question_raw[i], NormalizeInverse(g[i].cpu().data)])
            # pickle.dump(save_visual,open("save_visual","wb+"))
            # sys.exit()
            if compute_acc:
